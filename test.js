...
app.post("/getEventsFromDB", async (req, res) => {
  ...
  try {
    ...
    if (searchVal === null || searchVal === "") {
      ...
      await findEventsViaRegex(searchVal).then((eventsFromDB) => {
        console.log("events: ", eventsFromDB);
...
describe("Check Routes", function () {
  ...
  test("look for an item in catlaog", async () => {
    const res = await request(app).post("/itemsExistInCart").send({
      name: "Nike Air",
    });
...
describe("Check Routes", function () {
  ...
  test("add new item to cart", async () => {
    const res = await request(app).put(
      `/addItemToCart/${JSON.stringify(newItem)}`
    );
    expect(res.status).not.toBe(400);
  });
  ...
  test("Check for event in admin page", async () => {
    const res = await request(app).post("/getEventsFromDB").send({
      searchText: "admin",
    });
  ...
  test("Check for event not existing in the admin page", async () => {
    const res = await request(app).get("/getEventsFromDB").send({
